    ...
    public void render() {
        ...
        switch (state)
        {
            case STATE_MAIN:
                ...
                batch.draw(buttonRegion, 205, Gdx.graphics.getHeight() - 36, 45, 20);
                font.draw(batch, "Load", 212, Gdx.graphics.getHeight() - 20);
                batch.draw(buttonRegion, 260, Gdx.graphics.getHeight() - 36, map.isDirty() ? 57 : 45, 20);
                font.draw(batch, map.isDirty() ? "*Save*" : "Save", 267, Gdx.graphics.getHeight() - 20);
                break;
            case STATE_NEWSTAGE:
                font2.draw(batch, "Click on a grid intersection to create a new map", 16,
            ...
            case STATE_EDITNEWSTAGE:
                ...
                batch.draw(buttonRegion, 10, Gdx.graphics.getHeight() - 36, glyph.width + (ticks % 60 < 30 ? 10 : 15),
                        20);
        ...
        if (button == Buttons.LEFT)
        {
            switch (state)
            {
                case STATE_MAIN:
                    ...
                    {
                        if ((Gdx.graphics.getHeight() - screenY >= 12)
                                && (Gdx.graphics.getHeight() - screenY <= 12 + 16 * map.getHeight()))
                        {
                            int x = (screenX - 12) / 16;
